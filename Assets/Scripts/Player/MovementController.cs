    ...
    public class MovementController : MonoBehaviour
    {
        ...
        public void SetupMovement()
        {
            ...
            if (Input.GetKey(KeyCode.LeftShift) && !Values.GetIsTouchingWall() && !Values.GetIsTouchingWallWithHead() &&
                (Values.GetRigidbody().velocity.magnitude > 0) && (axisMovementVertical > 0))
            {
        ...
        public void MakeMovement()
        {
            ...
            if (Values.GetIsTouchingWallWithHead())
            {
                if (Values.GetRigidbody().position.y < Values.GetLastGoodYPosition())
                    Values.GetRigidbody().MovePosition(transform.position +
                                    (movementVec / 4) *
                                    Time.fixedDeltaTime); //lo rende talmente lento da farlo diventare un non problema
                else
        ...
        {
            if (Input.GetKey("space") && Values.GetStamina() >= 5 &&
                (Values.GetIsGrounded() || Values.GetIsInWater()) && !Values.GetIsTouchingWallWithHead())
            {
